...
def api_request(url, data=None, token=None, https_proxy=None, method=None):
    try:
        ...
        logError("Github Notifications: Error while contacting GitHub: cURL returned %d" % err.returncode)
    except EnvironmentError as err:
        traceback.print_exc()
        if type(err) == OSError and err.errno == 2 and api_request == api_request_curl:
            logError("Github Notifications: Unable to find Python SSL module or cURL")
        else:
            msg = "Github Notifications: Error while contacting GitHub"
            if err.strerror:
                msg += err.strerror
            logError(msg)
    except SimpleHTTPError as err:
        msg = "Github Notifications: GitHub returned error %d" % err.code
        try:
